...
class Admins(View):
    ...
    def post(self,request):
        ...
        print(app)
        if (app=='Approves'):
                obj=registration.objects.get(id=request.POST['check'])
                obj2=registration.objects.filter(id=request.POST['check']).update(status='approved')
                obj1=User.objects.create_user(username=obj.First_name,password=obj.password,email=obj.email)
                obj1.save()
                message='Admin'
                body= f"{obj.First_name} you are approved  "
                mail=settings.EMAIL_HOST_USER
                res=[obj.email]
                send_mail(message,body,mail,res)
            ...
            obj2=registration.objects.filter(id=request.POST['check']).update(status='rejected')
